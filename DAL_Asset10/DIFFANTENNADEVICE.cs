//------------------------------------------------------------------------------
// <auto-generated>
//     This code was generated from a template.
//
//     Manual changes to this file may cause unexpected behavior in your application.
//     Manual changes to this file will be overwritten if the code is regenerated.
// </auto-generated>
//------------------------------------------------------------------------------

namespace DAL_Asset10
{
    using System;
    using System.Collections.Generic;
    
    public partial class DIFFANTENNADEVICE
    {
        public decimal PROJECTNO { get; set; }
        public decimal DPROJECTNO { get; set; }
        public decimal DEVICEPK { get; set; }
        public decimal DDEVICEPK { get; set; }
        public string IDNAME { get; set; }
        public string DIDNAME { get; set; }
        public string DESCRIPTION { get; set; }
        public string DDESCRIPTION { get; set; }
        public string PHOTOFILE { get; set; }
        public string DPHOTOFILE { get; set; }
        public string MANUFACTURER { get; set; }
        public string DMANUFACTURER { get; set; }
        public Nullable<decimal> SUPPLIERKEY { get; set; }
        public Nullable<decimal> DSUPPLIERKEY { get; set; }
        public Nullable<decimal> PARENTKEY { get; set; }
        public Nullable<decimal> DPARENTKEY { get; set; }
        public Nullable<decimal> NOELEMENTS { get; set; }
        public Nullable<decimal> DNOELEMENTS { get; set; }
        public Nullable<decimal> ANTENNACATEGORY { get; set; }
        public Nullable<decimal> DANTENNACATEGORY { get; set; }
        public Nullable<decimal> COST { get; set; }
        public Nullable<decimal> DCOST { get; set; }
        public Nullable<decimal> WEIGHT { get; set; }
        public Nullable<decimal> DWEIGHT { get; set; }
        public Nullable<decimal> CREATEUSER { get; set; }
        public Nullable<decimal> DCREATEUSER { get; set; }
        public Nullable<decimal> USERGROUP { get; set; }
        public Nullable<decimal> DUSERGROUP { get; set; }
        public Nullable<System.DateTime> CREATEDATE { get; set; }
        public Nullable<System.DateTime> DCREATEDATE { get; set; }
        public Nullable<System.DateTime> MODIFYDATE { get; set; }
        public Nullable<System.DateTime> DMODIFYDATE { get; set; }
        public decimal MODIFYUSER { get; set; }
        public decimal DMODIFYUSER { get; set; }
        public Nullable<decimal> PERMISSION { get; set; }
        public Nullable<decimal> DPERMISSION { get; set; }
        public Nullable<decimal> DIFFSTATE { get; set; }
        public Nullable<decimal> DDEPTH { get; set; }
        public Nullable<decimal> DEPTH { get; set; }
        public Nullable<decimal> DHEIGHT { get; set; }
        public Nullable<decimal> DWIDTH { get; set; }
        public Nullable<decimal> HEIGHT { get; set; }
        public Nullable<decimal> WIDTH { get; set; }
        public Nullable<decimal> DNUMSSBBEAMS { get; set; }
        public Nullable<decimal> NUMSSBBEAMS { get; set; }
    }
}
